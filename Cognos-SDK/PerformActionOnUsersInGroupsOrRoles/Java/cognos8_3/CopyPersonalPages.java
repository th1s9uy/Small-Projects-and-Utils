 ...
 *
 * Description: (KB 1011181) - How to define all exisitng Groups and Roles and then perform an action on the associated members - Cognos 8.3
 */
...
public class CopyPersonalPages 
{
	...
	public void updatePages( String targetSearchPath, Pagelet templatePortalPage) 
	{
		...
		try {
			// get the target user's account object. 
			 targets = cmService.query(spMulti, props, sOpt, qOpt);
	...
	public void setPortalPages(BaseClass[] portalPages, Account targetAccount)
	{
		...
		try {
			...
			BaseClass temp[] = cmService.update(new BaseClass[]{targetAccount},uo);
	...
	public BaseClass[] copyPortalPage(String targetPath, Pagelet templatePortalPage)
	{
		...
		try {
			copies = cmService.copy(new BaseClass [] {templatePortalPage}, 
					new SearchPathSingleObject(targetPath), co);
	...
	public Pagelet getTemplatePage(String templateSearchPath, String pageName)
	{
		...
		SearchPathMultipleObject spMulti = new SearchPathMultipleObject();
		spMulti.setValue(templateSearchPath);
		
		try
		{
			// get the template user's Account object. 
			BaseClass template[] = cmService.query(spMulti, props, sOpt, qOpt);
			if (template != null && template.length >0)
			{
				...
				for (int i=0; i<pages.length ; i++)
				{
					...
					{
						spMulti.setValue(pages[i].getSearchPath().getValue());
						return (Pagelet)cmService.query(spMulti, props, sOpt, qOpt)[0]; 
					}
				}
			}else
	...
	{
		// Variable that contains the default URL for C8 Content Manager.			
